...
public class Pobieracz {
	...
	private static final int CZAS_MAKSYMALNY = 5;
	...
	private static final Pattern regexPrzebiegu = Pattern.compile("(\\d+\\s*)+km");
	...
	private void wczytajOgloszeniaZJednejStrony() {
	    ...
	    for(WebElement oferta : oferty) {
	    	try {
		    	...
		    	List<WebElement> elementyPrzebieg = oferta.findElements(By.cssSelector("li[title='Przebieg']"));
		    	int przebieg = 0;
		    	if(! elementyPrzebieg.isEmpty()) {
			    	Matcher matcherPrzebiegu = regexPrzebiegu.matcher(elementyPrzebieg.get(0).getText());
			    	if(matcherRocznika.find()) {
			    		przebieg = Integer.parseInt(matcherPrzebiegu.group(1).replace(" ", ""));
			    	}
		    	}

		    	WebElement elementSilnik = oferta.findElement(By.cssSelector("li[title='Pojemność silnika']"));
		    	String silnik = elementSilnik.getText();
		    	silnik = silnik.replace("Silnik:", "");
		    	
		    	Ogloszenie ogloszenie = new Ogloszenie(tytul, cena, rocznik, 0, silnik, url);
